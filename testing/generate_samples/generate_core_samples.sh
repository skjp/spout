...
outfile="${script_dir}/../tests/test_${datetime}.txt"
...
for module in "${modules[@]}"; do
    ...
    initial_passes=$(count_occurrences "\[PASS\]" "$outfile")
    initial_fails=$(count_occurrences "\[FAIL\]" "$outfile")
    let initial_passes=${initial_passes:-0}
    let initial_fails=${initial_fails:-0}
    
    # Run tests and capture output
    spout "$module" -x >> "$outfile"
    
    # Get current counts
    current_passes=$(count_occurrences "\[PASS\]" "$outfile")
    current_fails=$(count_occurrences "\[FAIL\]" "$outfile")
